...
1.	一般的な問題の一覧 (後述)を調べます。
2.	[**refman.doc**](refman.md) または [**library.doc**](library.md) など関連する説明書を読みます。
3.	次のコマンドを挿入してプログラムを実行してみてください。
...
5.	[Euphoria メーリングリストの検索システム](https://www.rapideuphoria.com/listserv.htm) を使用します。これにより過去の全メッセージのアーカイブを検索することができます。今回発生した問題は過去に質問された後に検討された可能性が十分あります。
...
%d のほかに、他の形式の使用を試すことができます。例えば%x (16進数) または %f (浮動小数点数)などがあります。
